...
{
	auto breakpointInspector = dynamic_cast<QDockWidget*>(CDockWidgetRegistry::instance()->getWidget("Breakpoints"));
	...
	if (!m_loadedTarget.compare("nes", Qt::CaseInsensitive))
	{
		ui->tableView->setCurrentIndex(
			assemblyViewModel->index(nesGetSLOCFromAddress(nesGetCPUProgramCounterOfLastSync()), 0));
	}
...
void CodeBrowserDockWidget::breakpointHit()
{
	if (isVisible())
	{
		if (nesROMIsLoaded())
		{
			...
			{
				ui->tableView->setCurrentIndex(
					assemblyViewModel->index(c64GetSLOCFromAddress(c64GetCPURegister(CPU_PC)), 0));
			}
			ui->tableView->scrollTo(ui->tableView->currentIndex());
			ui->tableView->resizeColumnsToContents();
		}
...
void CodeBrowserDockWidget::on_tableView_doubleClicked(QModelIndex index)
{
	if (index.isValid())
	{
		...
		item += assemblyViewModel->data(assemblyViewModel->index(index.row(), CodeBrowserCol_Address), Qt::DisplayRole).
		                           toString();
