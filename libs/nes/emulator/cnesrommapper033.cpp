...
void CROMMapper033::RESET (const bool soft )
{
   m_mapper = 33;
...
uint32_t CROMMapper033::DEBUGINFO (const uint32_t addr )
{
   switch ( addr&0xA003 )
   {
   case 0x8000:
...
void CROMMapper033::HMAPPER (const uint32_t addr, const uint8_t data )
{
   uint8_t bank;
