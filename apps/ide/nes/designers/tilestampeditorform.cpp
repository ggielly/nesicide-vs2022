...
TileStampEditorForm::TileStampEditorForm(QByteArray data, QByteArray attr, QString attrTblUUID,
                                         QList<PropertyItem> tileProperties, int xSize, int ySize, bool grid,
                                         IProjectTreeViewItem* link, QWidget* parent) :
	...
	ui(new Ui::TileStampEditorForm)
{
	...
	while (iter.current())
	{
		auto pChrItem = dynamic_cast<IChrRomBankItem*>(iter.current());
		auto pProjItem = dynamic_cast<IProjectTreeViewItem*>(iter.current());
		if (pChrItem &&
	...
	QObject::connect(tilePropertyListModel,SIGNAL(dataChanged(QModelIndex,QModelIndex)), this,
	                 SLOT(tilePropertyListModel_dataChanged(QModelIndex,QModelIndex)));
	QObject::connect(ui->propertyTableView->selectionModel(),SIGNAL(currentChanged(QModelIndex,QModelIndex)), this,
	                 SLOT(propertyTableView_currentChanged(QModelIndex,QModelIndex)));
	...
	if (item)
	{
		...
		if (pAttrTbl)
		{
			...
			{
				m_colors.at(idx)->setCurrentColor(QColor(nesGetPaletteRedComponent(pAttrTbl->getPalette().at(idx)),
				                                         nesGetPaletteGreenComponent(pAttrTbl->getPalette().at(idx)),
				                                         nesGetPaletteBlueComponent(pAttrTbl->getPalette().at(idx))));
			}
		}
	}
	else
	{
		...
		{
			m_colors.at(idx + 0)->setCurrentColor(QColor(nesGetPaletteRedComponent(0x0D),
			                                             nesGetPaletteGreenComponent(0x0D),
			                                             nesGetPaletteBlueComponent(0x0D)));
			m_colors.at(idx + 1)->setCurrentColor(QColor(nesGetPaletteRedComponent(0x00),
			                                             nesGetPaletteGreenComponent(0x00),
			                                             nesGetPaletteBlueComponent(0x00)));
			m_colors.at(idx + 2)->setCurrentColor(QColor(nesGetPaletteRedComponent(0x10),
			                                             nesGetPaletteGreenComponent(0x10),
			                                             nesGetPaletteBlueComponent(0x10)));
			m_colors.at(idx + 3)->setCurrentColor(QColor(nesGetPaletteRedComponent(0x20),
			                                             nesGetPaletteGreenComponent(0x20),
			                                             nesGetPaletteBlueComponent(0x20)));
		}
	}
	...
	QObject::connect(ui->attributeTable,SIGNAL(currentIndexChanged(int)), this,
	                 SLOT(attributeTable_currentIndexChanged(int)));
...
void TileStampEditorForm::keyPressEvent(QKeyEvent* event)
{
	...
	if (event->modifiers() == Qt::ControlModifier)
	{
		if (event->key() == Qt::Key_C)
		{
			...
			{
				paintOverlay(Overlay_PasteSelection, 0, m_selectionRect.left(), m_selectionRect.top(),
				             m_selectionRect.right(), m_selectionRect.bottom());
				copyOverlayToNormal();
				renderer->update();
				previewer->update();
				copySelectionToClipboard(m_selectionRect.left(), m_selectionRect.top(), m_selectionRect.right(),
		...
		else if (event->key() == Qt::Key_V)
		{
			...
			if (!(clipboard->image().isNull()))
			{
				...
				m_selectionRect = QRect(image.rect().left(), image.rect().top(), image.rect().width() + 1,
				                        image.rect().height() + 1);
				renderer->setBox(m_selectionRect.left(), m_selectionRect.top(), m_selectionRect.right(),
				                 m_selectionRect.bottom());
				m_selection = true;
				renderer->update();
				previewer->update();
		...
		else if (event->key() == Qt::Key_X)
		{
			if (m_selection && m_selectionCaptured)
			{
				...
				copySelectionToClipboard(m_selectionRect.left(), m_selectionRect.top(), m_selectionRect.right(),
				                         m_selectionRect.bottom());
				paintOverlay(Overlay_Erase, 0, m_selectionRect.left(), m_selectionRect.top(), m_selectionRect.right(),
				             m_selectionRect.bottom());
				copyOverlayToNormal();
...
void TileStampEditorForm::paintOverlay(QByteArray overlayData, QByteArray overlayAttr, int overlayXSize,
                                       int overlayYSize, int boxX1, int boxY1, int boxX2, int boxY2)
{
	int idx;
	QColor color;
	int x;
			...
			if (((posY + tileY) < boxY2) && ((posX + tileX) < boxX2))
			{
				for (y = 0; y < 8; y++)
				{
					...
					for (x = 0; x < 8; x++)
					{
						...
						if (!recolorPointQueue.contains(QPoint(posX + tileX + x - ((posX + tileX + x) % 16),
						                                       posY + tileY + y - ((posY + tileY + y) % 16))))
						{
							recolorPointQueue.append(QPoint(posX + tileX + x - ((posX + tileX + x) % 16),
							                                posY + tileY + y - ((posY + tileY + y) % 16)));
...
void TileStampEditorForm::paintOverlay(OverlayType type, int selectedColor, int boxX1, int boxY1, int boxX2, int boxY2)
{
	...
	//4. While Q is not empty:
		while (!fillQueue.isEmpty())
		{
			...
			if (selectedColor == 0)
			{
				colorDataOverlay[(point.y() * 256) + point.x()] = (colorDataOverlay[(point.y() * 256) + point.x()]) &
					0xFC;
			}
			else
			{
				...
				(colorDataOverlay[(point.y() * 256) + point.x() - 1] == targetColor))
			{
				...
				if (selectedColor == 0)
				{
					colorDataOverlay[(point.y() * 256) + point.x() - 1] = (colorDataOverlay[(point.y() * 256) + point.
						x() - 1]) & 0xFC;
				}
				else
				{
				...
				if (!recolorQueue.contains(QPoint((point.x() - 1) - ((point.x() - 1) % 16),
				                                  point.y() - (point.y() % 16))))
				{
				...
				(colorDataOverlay[((point.y() - 1) * 256) + point.x()] == targetColor))
			{
				...
				if (selectedColor == 0)
				{
					colorDataOverlay[((point.y() - 1) * 256) + point.x()] = (colorDataOverlay[((point.y() - 1) * 256) +
						point.x()]) & 0xFC;
				}
				else
				{
				...
				if (!recolorQueue.contains(QPoint(point.x() - (point.x() % 16),
				                                  (point.y() - 1) - ((point.y() - 1) % 16))))
				{
			...
			for (y = -radius; y <= radius; y++)
			{
				for (x = -radius; x <= radius; x++)
				{
					if (x * x + y * y <= radius * radius)
					{
						...
						if (selectedColor == 0)
						{
							colorDataOverlay[((boxY1 + ((boxY2 - boxY1) / 2) + y) * 256) + (boxX1 + ((boxX2 - boxX1) /
								2) + x)] = (colorDataOverlay[((boxY1 + ((boxY2 - boxY1) / 2) + y) * 256) + (boxX1 + ((
								boxX2 - boxX1) / 2) + x)]) & 0xFC;
						}
						else
						{
							colorDataOverlay[((boxY1 + ((boxY2 - boxY1) / 2) + y) * 256) + (boxX1 + ((boxX2 - boxX1) /
	...
	for (tileY = 0; tileY < tileHeight; tileY++)
	{
		for (tileX = 0; tileX < tileWidth; tileX++)
		{
			...
			if (((tileX & 0x1) == 0) && ((tileY & 0x1) == 0))
			{
				if (useOverlay)
				{
					attrTable[((tileY >> 1) * (tileWidth >> 1)) + (tileX >> 1)] = colorDataOverlay[(tileY * 8 * 256) + (
						tileX * 8)] >> 2;
				}
				else
				{
					attrTable[((tileY >> 1) * (tileWidth >> 1)) + (tileX >> 1)] = colorData[(tileY * 8 * 256) + (tileX *
						8)] >> 2;
				}
			}
		}
...
void TileStampEditorForm::selectionTool(QMouseEvent* event)
{
	...
	case QEvent::MouseButtonRelease:
		if (m_selection)
		{
			...
			{
				copyNormalToSelection(m_selectionRect.left(), m_selectionRect.top(), m_selectionRect.right(),
				                      m_selectionRect.bottom());
				paintOverlay(Overlay_Erase, 0, m_selectionRect.left(), m_selectionRect.top(), m_selectionRect.right(),
				             m_selectionRect.bottom());
				copyOverlayToNormal();
...
void TileStampEditorForm::tileTool(QMouseEvent* event)
{
	...
	int boxX2;
	int boxY2;
	auto pChrItem = reinterpret_cast<IChrRomBankItem*>(ui->tileTabWidget->currentIndex().internalPointer());
	auto pSelectedTile = dynamic_cast<CTileStamp*>(pChrItem);
	auto pSelectedBinary = dynamic_cast<CBinaryFile*>(pChrItem);
	int xSize;
	int ySize;
	QByteArray oldTileData;
	QByteArray oldAttributeData;
	...
	case QEvent::MouseButtonPress:
		if (pSelectedTile)
		{
			...
			paintOverlay(pSelectedTile->getTileData(), pSelectedTile->getAttributeData(), xSize, ySize, boxX1, boxY1,
			             boxX2, boxY2);
			renderer->setBox(boxX1, boxY1, boxX2, boxY2);
			renderer->update();
			previewer->update();
	...
	case QEvent::MouseMove:
		if (event->buttons() == Qt::LeftButton)
		{
			if (pSelectedTile)
			{
				...
				{
					paintOverlay(pSelectedTile->getTileData(), pSelectedTile->getAttributeData(), xSize, ySize, boxX1,
					             boxY1, boxX2, boxY2);
					renderer->setBox(boxX1, boxY1, boxX2, boxY2);
					renderer->update();
					previewer->update();
			...
			else if (pSelectedBinary)
			{
				...
				if ((boxY2 > boxY1) && (boxX2 > boxX1))
				{
					paintOverlay(pSelectedBinary->getBinaryData(), QByteArray(), xSize, ySize, boxX1, boxY1, boxX2,
					             boxY2);
					renderer->setBox(boxX1, boxY1, boxX2, boxY2);
					renderer->update();
					previewer->update();
