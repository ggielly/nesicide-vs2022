...
{
	// Tell the world.
	auto emulator = dynamic_cast<C64EmulatorThread*>(CObjectRegistry::instance()->getObject("Emulator"));
...
C64EmulatorThread::C64EmulatorThread(QObject*)
{
	...
	QObject::connect(m_pViceApp,SIGNAL(finished(int,QProcess::ExitStatus)), this,
	                 SLOT(viceFinished(int,QProcess::ExitStatus)));
	...
	auto breakpointWatcher = dynamic_cast<BreakpointWatcherThread*>(CObjectRegistry::instance()->getObject(
		"Breakpoint Watcher"));
...
C64EmulatorThread::~C64EmulatorThread()
{
	...
	QObject::disconnect(m_pViceApp,SIGNAL(finished(int,QProcess::ExitStatus)), this,
	                    SLOT(viceFinished(int,QProcess::ExitStatus)));
...
void C64EmulatorThread::viceStarted()
{
	...
	QObject::connect(m_pClient,SIGNAL(responses(QStringList,QStringList)), this,
	                 SLOT(processResponses(QStringList,QStringList)));
	QObject::connect(this,SIGNAL(sendRequests(QStringList,QList<int>)), m_pClient,
	                 SLOT(sendRequests(QStringList,QList<int>)));
...
void C64EmulatorThread::resetEmulator()
{
	...
	if (CNesicideProject::instance()->isInitialized())
	{
		...
		QString fileName = dirProject.toNativeSeparators(
			dirProject.absoluteFilePath(CNesicideProject::instance()->getProjectLinkerOutputName()));
		QString request;
		int addr;
	  ...
	  m_port(monitorPort)
{
	...
	QObject::connect(pSocket,SIGNAL(error(QAbstractSocket::SocketError)), this,
