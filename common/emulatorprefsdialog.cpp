...
void EmulatorPrefsDialog::readSettings()
{
	...
	for (port = 0; port < NUM_CONTROLLERS; port++)
	{
		...
		for (function = 0; function < IO_Zapper_MAX; function++)
		{
			...
			zapperMouseMap[port][function] = settings.value("MouseMap" + QString::number(function), QVariant(true)).
			                                          toBool();
		}
		settings.endGroup();
		...
		for (function = 0; function < IO_Vaus_MAX; function++)
		{
			...
			vausArkanoidMouseMap[port][function] = settings.value("MouseMap" + QString::number(function),
...
void EmulatorPrefsDialog::updateUi()
{
	...
	ui->controllerKeysStackedWidget->setCurrentWidget(
		controllerPageMap.value(ui->controllerTypeComboBox->currentText()));
