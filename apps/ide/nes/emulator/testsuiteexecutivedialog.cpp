...
void TestSuiteExecutiveDialog::loadTestSuite(QString testSuiteFileName)
{
	...
	ui->testSuiteFileName->setText(
		testSuiteFolder.toNativeSeparators(testSuiteFolder.absoluteFilePath(testSuiteFileName)));
	...
	if (!testSuiteFileName.isEmpty())
	{
		...
		{
			settings.setValue("TestSuiteFile",
			                  testSuiteFolder.toNativeSeparators(testSuiteFolder.absoluteFilePath(testSuiteFileName)));
	...
	headerLabels << "File Name" << "# of Frames" << "System" << "Last Result" << "Failure Comment" << "Notes" <<
		"TV SHA1" << "Recorded Input";
...
{
	QSettings settings(QSettings::IniFormat, QSettings::UserScope, "CSPSoftware", "NESICIDE");
	QString testSuiteFileName = QFileDialog::getOpenFileName(this, "Test Suite XML File",
	                                                         settings.value("TestSuiteFile").toString(),
	                                                         "XML Files (*.xml)");
	...
	case 2:
		...
		if (crypto.result().toBase64() != previousSha1.toLatin1())
		{
			...
			dlg.setInformativeText(
				"To add more time to the test run, click \"Retry\".\nTo abort test suite, click \"Abort\".");
			...
			{
				testFailComment = QInputDialog::getText(this, testFileName + ": Enter failure comment", "Reason:",
				                                        QLineEdit::Normal, testFailComment);
			}
...
{
	QSettings settings(QSettings::IniFormat, QSettings::UserScope, "CSPSoftware", "NESICIDE");
	QString testSuiteFileName = QFileDialog::getSaveFileName(this, "Test Suite XML File",
	                                                         settings.value("TestSuiteFile").toString(),
	                                                         "XML Files (*.xml)");
	QDir testSuiteFolder(testSuiteFileName);
	QDomElement testSuiteElement;
	...
	if (!testSuiteFileName.isEmpty())
	{
		...
		if (testSuiteFile.open(QIODevice::ReadWrite | QIODevice::Text | QIODevice::Truncate))
		{
			...
			for (test = 0; test < numTests; test++)
			{
				...
				dataSect = testSuiteDoc.createCDATASection(
					ui->tableWidget->item(test, 7)->data(Qt::UserRole).toString());
				childElement.appendChild(dataSect);
			}
