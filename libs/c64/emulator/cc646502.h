...
public:
   ...
   static void DISASSEMBLE ( char** disassembly, const uint8_t* binary, int32_t binaryLength, const uint8_t* opcodeMask, uint16_t* sloc2addr, uint32_t* addr2sloc, uint32_t* sourceLength );
   static char* Disassemble (const uint8_t* pOpcode, char* buffer );
   ...
   static void GOTO (const uint32_t pcGoto )
   {
      m_pcGoto = pcGoto;
   }
   ...
   static void __PC (const uint16_t pc )
   {
      m_pc = pc;
   }
   ...
   static void _SP (const uint8_t sp )
   {
      m_sp = sp;
   }
   ...
   static void _A (const uint8_t a )
   {
      m_a = a;
   }
   ...
   static void _X (const uint8_t x )
   {
      m_x = x;
   }
   ...
   static void _Y (const uint8_t y )
   {
      m_y = y;
   }
   ...
   // disassembler.
   static inline void OPCODEMASK (const uint32_t addr, const uint8_t mask )
   {
      *(m_RAMopcodeMask+addr) = mask;
   }
   static inline void OPCODEMASKCLR ( void )
   {
	   for ( int32_t idx = 0; idx < MEM_64KB; idx++ )
      {
         m_RAMopcodeMask[idx] = 0;
      }
   }
   static inline char* DISASSEMBLY (const uint32_t addr )
   {
      return *(m_RAMdisassembly+addr);
   }
   static inline char* DISASSEMBLYATPHYSADDR (const uint32_t absAddr, char* buffer )
   {
      return cc646502::Disassemble(m_6502memory+absAddr,buffer);
   }
   static uint16_t SLOC2VIRTADDR (const uint32_t sloc )
   {
      return *(m_RAMsloc2addr+sloc);
   }
   static uint32_t ADDR2SLOC (const uint16_t addr )
   {
