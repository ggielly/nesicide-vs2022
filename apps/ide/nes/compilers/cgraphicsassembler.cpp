...
bool CGraphicsAssembler::assemble()
{
	...
	if (CNesicideProject::instance()->getProjectCHRROMOutputName().isEmpty())
	{
		outputName = outputDir.fromNativeSeparators(
			outputDir.filePath(CNesicideProject::instance()->getProjectOutputName() + ".chr"));
	}
	else
	{
		outputName = outputDir.fromNativeSeparators(
			outputDir.filePath(CNesicideProject::instance()->getProjectCHRROMOutputName()));
	}
			...
			for (int chrBankIdx = 0, gfxBankIdx = 0; gfxBankIdx < gfxBanks->getGraphicsBanks().count(); gfxBankIdx++)
			{
				...
				if (curGfxBank->getGraphics().count())
				{
					for (int bankItemIdx = 0; bankItemIdx < curGfxBank->getGraphics().count(); bankItemIdx++)
					{
						...
						auto ptvi = dynamic_cast<IProjectTreeViewItem*>(bankItem);
						buildTextLogger->write(
							"&nbsp;&nbsp;&nbsp;Adding: " + ptvi->caption() + "(" + QString::number(
								bankItem->getChrRomBankItemSize()) + " bytes)");
						...
						{
							buildTextLogger->write(
								"<font color='red'>" + curGfxBank->caption() + "(" + QString::number(gfxBankSize) +
								"): Warning: too much data for specified size</font>");
					...
					(currentChrSize >= MEM_8KB))
				{
					...
					{
						auto curBank = new CCHRROMBank(CNesicideProject::instance()->getCartridge()->getChrRomBanks());
						// This is a new bank
