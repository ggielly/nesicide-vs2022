...
void MainWindow::createNesUi()
{
	...
	m_pGfxCHRMemoryInspector->setFeatures(
		...
		else
		{
			if (level > 0)
			{
					...
					localFile.open(QIODevice::ReadWrite | QIODevice::Truncate))
				{
					...
					{
						CSourceItem* source = CNesicideProject::instance()->getProject()->getSources()->addSourceFile(
							projectDirPath);
...
void MainWindow::on_actionCreate_Project_from_File_triggered()
{
	...
	else if (fileName.endsWith(".d64", Qt::CaseInsensitive) || fileName.endsWith(".prg", Qt::CaseInsensitive) ||
		fileName.endsWith(".c64", Qt::CaseInsensitive))
	{
		openC64File(fileName);
	}
}
