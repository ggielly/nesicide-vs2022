...
QImage CImageConverters::toIndexed8(QByteArray chrIn, QList<int> colorTable, int xSize, int ySize)
{
	...
	{
		imgOut.setColor(idx, qRgb(CBasePalette::GetPaletteR(colorTable.at(idx)),
		                          CBasePalette::GetPaletteG(colorTable.at(idx)),
		                          CBasePalette::GetPaletteB(colorTable.at(idx))));
	}
...
QImage CImageConverters::toIndexed8(QByteArray chrIn, QByteArray attrIn, QList<uint8_t> colorTable, int xSize,
                                    int ySize)
{
	QImage imgOut(xSize, ySize, QImage::Format_Indexed8);
