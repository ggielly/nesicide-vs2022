	...
	ui(new Ui::SearchWidget)
{
	...
	QObject::connect(this,SIGNAL(search(QDir,QString,QString,bool,bool,bool,bool)), searcher,
	                 SLOT(search(QDir,QString,QString,bool,bool,bool,bool)));
...
void SearchWidget::on_find_clicked()
{
	QSettings settings(QSettings::IniFormat, QSettings::UserScope, "CSPSoftware", "NESICIDE");
	QStringList items;
	auto searcher = dynamic_cast<SearcherThread*>(CObjectRegistry::instance()->getObject("Searcher"));
	...
	if (!ui->searchText->currentText().isEmpty())
	{
		...
		if ((!ui->location->currentText().isEmpty()) && (ui->location->findText(
			ui->location->currentText(), Qt::MatchExactly | Qt::MatchCaseSensitive) == -1))
		{
		...
		if ((!ui->type->currentText().isEmpty()) && (ui->type->findText(
			ui->type->currentText(), Qt::MatchExactly | Qt::MatchCaseSensitive) == -1))
		{
		...
		{
			emit search(QDir::currentPath(), ui->searchText->currentText(), ui->type->currentText(),
			            ui->subfolders->isChecked(), ui->sourceSearchPaths->isChecked(), ui->regex->isChecked(),
			            ui->caseSensitive->isChecked());
		}
		else
		{
			emit search(ui->location->currentText(), ui->searchText->currentText(), ui->type->currentText(),
			            ui->subfolders->isChecked(), ui->sourceSearchPaths->isChecked(), ui->regex->isChecked(),
			            ui->caseSensitive->isChecked());
		}
