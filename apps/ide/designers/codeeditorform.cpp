	...
	m_apis(nullptr)
{
	...
	auto symbolWatch = dynamic_cast<QDockWidget*>(CDockWidgetRegistry::instance()->getWidget("Symbol Inspector"));
	auto executionVisualizer = dynamic_cast<QDockWidget*>(CDockWidgetRegistry::instance()->getWidget(
	...
	QObject::connect(m_scintilla,SIGNAL(customContextMenuRequested(const QPoint&)), this,
	...
	for (marker = 0; marker < markers->GetNumMarkers(); marker++)
	{
		...
		m_scintilla->setMarkerBackgroundColor(QColor(pMarker->red, pMarker->green, pMarker->blue),
		                                      Marker_Marker1 + marker);
		m_scintilla->setMarkerForegroundColor(QColor(pMarker->red, pMarker->green, pMarker->blue),
		                                      Marker_Marker1 + marker);
	}
	...
	QObject::connect(m_scintilla,SIGNAL(marginClicked(int,int,Qt::KeyboardModifiers)), this,
	                 SLOT(editor_marginClicked(int,int,Qt::KeyboardModifiers)));
	...
	QObject::connect(m_scintilla,SIGNAL(cursorPositionChanged(int,int)), this,
	                 SLOT(editor_cursorPositionChanged(int,int)));
...
{
	auto TOOLTIP_LABEL = "<b>%s</b><br>Location: %s(%d)<br>Address: %s<br>Value: %02X";
	auto TOOLTIP_EQUATE = "<b>%s</b>(%X)";
	...
	if (EnvironmentSettingsDialog::showSymbolTips())
	{
			...
			(CCC65Interface::instance()->getSymbolType(symbol) == CC65_SYM_IMPORT))
		{
			...
			if (addr != 0xFFFFFFFF)
			{
				...
				if (!CNesicideProject::instance()->getProjectTarget().compare("nes", Qt::CaseInsensitive))
				{
					sprintf(toolTipText, TOOLTIP_LABEL, symbol.toLatin1().constData(), file.toLatin1().constData(),
					        line, address, nesGetMemory(addr));
				}
				else if (!CNesicideProject::instance()->getProjectTarget().compare("c64", Qt::CaseInsensitive))
				{
					sprintf(toolTipText, TOOLTIP_LABEL, symbol.toLatin1().constData(), file.toLatin1().constData(),
					        line, address, c64GetMemory(addr));
				}
					...
					for (; addr <= endAddr; addr++, absAddr++)
					{
						if (CCC65Interface::instance()->isPhysicalAddressAnOpcode(absAddr))
						{
							...
							if (disassembly[0])
							{
								...
								if (!CNesicideProject::instance()->getProjectTarget().compare(
									"nes", Qt::CaseInsensitive))
								{
									nesGetPrintablePhysicalAddress(address, addr, absAddr);
								}
								else if (!CNesicideProject::instance()->getProjectTarget().compare(
	...
	if (from.startsWith("SearchBar,"))
	{
		if (isVisible())
		{
				...
				(splits.at(4) != m_scintilla->selectedText()))
			{
				...
				found = m_scintilla->findFirst(splits.at(4), splits.at(2).toInt(), splits.at(1).toInt(), false, true,
				                               splits.at(3).toInt(), line, index);
			}
