...
void SearcherWorker::search(QDir dir, QString searchText, QString pattern, bool subfolders, bool sourceSearchPaths,
		...
		else if (entries.at(entry).isFile())
		{
			...
			if (file.open(QIODevice::ReadOnly))
			{
				...
				for (line = 0; line < contentLines.count(); line++)
				{
					...
					{
						foundText.sprintf(
							"%s:%d:%s", base.relativeFilePath(entries.at(entry).filePath()).toLatin1().constData(),
							line + 1, contentLines.at(line).toLatin1().constData());
