...
RegisterInspectorDockWidget::RegisterInspectorDockWidget(regDBFunc regDB, CBreakpointInfo* pBreakpoints,
                                                         QWidget* parent) :
	...
	ui(new Ui::RegisterInspectorDockWidget)
{
   ...
   ui->bitfieldView->setFont(QFont("Monospace", 10));
	...
	QObject::connect(ui->binaryView->selectionModel(), SIGNAL(currentChanged(QModelIndex,QModelIndex)), this,
	                 SLOT(binaryView_currentChanged(QModelIndex,QModelIndex)));
}
...
void RegisterInspectorDockWidget::updateMemory()
{
	...
	if (m_regDB)
	{
		sprintf(buffer, "%04X: %s", m_regDB->GetRegister(m_register)->GetAddr(),
		        m_regDB->GetRegister(m_register)->GetName());
