...
GraphicsBankEditorForm::GraphicsBankEditorForm(uint32_t size, QList<IChrRomBankItem*> bankItems,
...
void GraphicsBankEditorForm::renderData(QByteArray output)
{
	...
	if (output.count() > max)
	{
		...
		ui->gauge->setStyleSheet(
			"QProgressBar::chunk {background: #ff0000;border-top-right-radius: 5px;border-top-left-radius: 5px;border-bottom-right-radius: 5px;border-bottom-left-radius: 5px;border: .px solid black;}");
	}
	else
	{
		ui->gauge->setStyleSheet(
			"QProgressBar::chunk {background: #00ff00;border-top-right-radius: 5px;border-top-left-radius: 5px;border-bottom-right-radius: 5px;border-bottom-left-radius: 5px;border: .px solid black;}");
	}
...
void GraphicsBankEditorForm::snapTo(QString item)
{
	...
	if (item.startsWith("SearchBar,"))
	{
		if (isVisible())
		{
			...
			list = model->match(model->index(ui->tableView->currentIndex().row() + 1, ChrRomBankItemCol_Name),
			                    Qt::DisplayRole, splits.at(4), 1,
			                    Qt::MatchFixedString | Qt::MatchContains | Qt::MatchWrap);
			if (list.count())
			{
