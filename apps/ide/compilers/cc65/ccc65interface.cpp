...
QStringList CCC65Interface::getAssemblerSourcesFromProject()
{
	...
	QStringList extensions = EnvironmentSettingsDialog::sourceExtensionsForAssembly().split(
...
bool CCC65Interface::createMakefile()
{
	...
	if (res.isOpen() && makeFile.isOpen())
	{
		...
		makeFileContent.replace("<!assembler-include-paths!>",
		                        CNesicideProject::instance()->getAssemblerIncludePaths());
		...
		makeFileContent.replace("<!linker-dependencies!>",
		                        CNesicideProject::instance()->getLinkerAdditionalDependencies());
...
void CCC65Interface::clean()
{
	...
	QObject::connect(&process,SIGNAL(errorOccurred(QProcess::ProcessError)), this,
	...
	QObject::connect(&process,SIGNAL(stateChanged(QProcess::ProcessState)), this,
	                 SLOT(process_stateChanged(QProcess::ProcessState)));
	...
	QObject::disconnect(&process,SIGNAL(finished(int,QProcess::ExitStatus)), this,
	                    SLOT(process_finished(int,QProcess::ExitStatus)));
	QObject::disconnect(&process,SIGNAL(errorOccurred(QProcess::ProcessError)), this,
	                    SLOT(process_errorOccurred(QProcess::ProcessError)));
	...
	QObject::disconnect(&process,SIGNAL(stateChanged(QProcess::ProcessState)), this,
	                    SLOT(process_stateChanged(QProcess::ProcessState)));
...
bool CCC65Interface::assemble()
{
	...
	{
		outputName = outputDir.fromNativeSeparators(
			outputDir.filePath(CNesicideProject::instance()->getProjectOutputName() + ".prg"));
	}
	else
	{
		outputName = outputDir.fromNativeSeparators(
			outputDir.filePath(CNesicideProject::instance()->getProjectLinkerOutputName()));
	}
...
bool CCC65Interface::captureDebugInfo()
{
	...
	{
		dbgInfoFile = dir.fromNativeSeparators(
			dir.relativeFilePath(CNesicideProject::instance()->getProjectOutputName() + ".dbg"));
	}
	else
	{
		dbgInfoFile = dir.fromNativeSeparators(
			dir.relativeFilePath(CNesicideProject::instance()->getProjectDebugInfoName()));
	}
...
bool CCC65Interface::captureINESImage()
{
	...
	{
		nesName = dir.fromNativeSeparators(
			dir.relativeFilePath(CNesicideProject::instance()->getProjectOutputName() + ".nes"));
	}
	else
	{
		nesName = dir.fromNativeSeparators(
...
{
	const cc65_symbolinfo* dbgSymbols;
	auto type = (cc65_symbol_type)CC65_INV_ID;
						...
						dbgLines && (dbgLines->count > 0))
					{
						if (dbgSegments->data[0].output_name)
						{
							if ((absAddr >= dbgSegments->data[0].output_offs - 0x10) &&
								(absAddr < (dbgSegments->data[0].output_offs - 0x10 + dbgSegments->data[0].
									segment_size)))
							{
	...
	if (dbgInfo)
	{
		...
		if (dbgSources)
		{
			...
			if (indexOfHighestTypeMatch >= 0)
			{
				...
				if (dbgSegments->data[0].output_name)
				{
					absAddr = dbgSegments->data[0].output_offs + (dbgSpans->data[0].span_start - dbgSegments->data[0].
						segment_start) - 0x10;
				}
				else
				{
	...
	if (dbgInfo)
	{
		...
		if (dbgSources)
		{
			...
			if (indexOfHighestTypeMatch >= 0)
			{
				...
				// Get segment information from span.
				dbgSegments = cc65_segment_byid(dbgInfo, dbgSpans->data[0].segment_id);
				absAddr = dbgSegments->data[0].segment_start + (dbgSpans->data[0].span_start - dbgSegments->data[0].
					segment_start);
		...
		if (dbgSpans)
		{
			for (span = 0; span < dbgSpans->count; span++)
			{
					...
					dbgLines && (dbgLines->count > 0))
				{
					if (dbgSegments->data[0].output_name)
					{
						if ((absAddr >= dbgSegments->data[0].output_offs + (dbgSpans->data[span].span_start -
								dbgSegments->data[0].segment_start) - 0x10) &&
							(absAddr <= dbgSegments->data[0].output_offs + (dbgSpans->data[span].span_end - dbgSegments
								->data[0].segment_start) - 0x10))
						{
							for (line = 0; line < dbgLines->count; line++)
							{
								if (dbgLines->data[line].line_type >= highestTypeMatch)
					...
					{
						if ((absAddr >= dbgSegments->data[0].output_offs + (dbgSpans->data[span].span_start -
								dbgSegments->data[0].segment_start)) &&
							(absAddr <= dbgSegments->data[0].output_offs + (dbgSpans->data[span].span_end - dbgSegments
								->data[0].segment_start)))
						{
							for (line = 0; line < dbgLines->count; line++)
							{
								if (dbgLines->data[line].line_type >= highestTypeMatch)
		...
		if (dbgSpans)
		{
			for (span = 0; span < dbgSpans->count; span++)
			{
				...
				if (dbgSegments && (dbgSegments->count == 1) &&
					dbgLines && (dbgLines->count > 0))
				{
					if ((absAddr >= dbgSegments->data[0].segment_start + (dbgSpans->data[span].span_start - dbgSegments
							->data[0].segment_start)) &&
						(absAddr <= dbgSegments->data[0].segment_start + (dbgSpans->data[span].span_end - dbgSegments->
							data[0].segment_start)))
					{
						for (line = 0; line < dbgLines->count; line++)
						{
