...
public:
	CBitfieldData(const char* name, char start, char width, const char* displayFormat, int values, ...)
	{
		...
		if (values)
		{
			...
			{
				auto temp = (char*)va_arg(va, const char*);
...
{
public:
	CRegisterDatabase(int type, int rows, int columns, int registers, CRegisterData** tblRegisters,
	                  const char** rowHeadings, const char** columnHeadings)
	{
		m_type = type;
