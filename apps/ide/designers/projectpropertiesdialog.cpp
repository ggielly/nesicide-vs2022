	...
	ui(new Ui::ProjectPropertiesDialog)
{
	...
	QObject::connect(ui->sourceSearchList->selectionModel(),SIGNAL(selectionChanged(QItemSelection,QItemSelection)),
	                 this,SLOT(sourceSearchList_selectionChanged(QItemSelection,QItemSelection)));
...
void ProjectPropertiesDialog::on_buttonBox_accepted()
{
	...
	CNesicideProject::instance()->getCartridge()->
	                              setMapperNumber(mapperIDFromIndex(ui->mapperComboBox->currentIndex()));
...
void ProjectPropertiesDialog::on_addSearchPath_clicked()
{
	...
	((QStringListModel*)ui->sourceSearchList->model())->setStringList(
		CNesicideProject::instance()->getSourceSearchPaths());
}
