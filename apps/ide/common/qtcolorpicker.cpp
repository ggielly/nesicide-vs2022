	...
	: QPushButton(parent), popup(nullptr), withColorDialog(enableColorDialog), withColorPopup(true)
{
	...
	{
		insertColor(QColor(nesGetPaletteRedComponent(i), nesGetPaletteGreenComponent(i), nesGetPaletteBlueComponent(i)),
		            "", i);
	}
}
...
QColor ColorPickerPopup::color(int index) const
{
	...
	auto that = (ColorPickerPopup*)this;
...
void ColorPickerPopup::updateSelected()
{
	...
	{
		auto item = (ColorPickerItem*)sender();
		lastSel = item->color();
