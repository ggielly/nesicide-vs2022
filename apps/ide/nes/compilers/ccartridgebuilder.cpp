...
bool CCartridgeBuilder::build()
{
	...
	{
		prgName = outputPRGDir.fromNativeSeparators(
			outputPRGDir.filePath(CNesicideProject::instance()->getProjectOutputName() + ".prg"));
	}
	else
	{
		prgName = outputPRGDir.fromNativeSeparators(
			outputPRGDir.filePath(CNesicideProject::instance()->getProjectLinkerOutputName()));
	}
	if (CNesicideProject::instance()->getProjectCHRROMOutputName().isEmpty())
	{
		chrName = outputCHRDir.fromNativeSeparators(
			outputCHRDir.filePath(CNesicideProject::instance()->getProjectOutputName() + ".chr"));
	}
	else
	{
		chrName = outputCHRDir.fromNativeSeparators(
			outputCHRDir.filePath(CNesicideProject::instance()->getProjectCHRROMOutputName()));
	}
	if (CNesicideProject::instance()->getProjectCartridgeOutputName().isEmpty())
	{
		nesName = baseDir.fromNativeSeparators(
			baseDir.relativeFilePath(CNesicideProject::instance()->getProjectOutputName() + ".nes"));
	}
	else
	{
		nesName = baseDir.fromNativeSeparators(
			baseDir.relativeFilePath(CNesicideProject::instance()->getProjectCartridgeOutputName()));
	}
		...
		(CCC65Interface::instance()->getCustomSourcesFromProject().count()))
	{
		...
		else
		{
			buildTextLogger->write(
				"<font color='red'>Nothing to build, no linker config file specified.  Go to Project menu, select Project Properties, and switch to the Linker tab to add one.</font>");
		}
	}
	else
	{
